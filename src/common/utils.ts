...
import { ProviderError, ErrorType } from './types.js';

export const validate_api_key = (key: string | undefined, provider: string): string => {
  if (!key) {
...
export const handle_rate_limit = (provider: string, reset_time?: Date): never => {
  throw new ProviderError(
